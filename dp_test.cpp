...
vector<vector<pair<int, pair<int, int> > > >    diskRequestSegment;
...
double GetDPvalue(int diskNum, int bth, int reqSeg, int curVer){
    ...
    int reqVer = diskRequestSegment[diskNum][reqSeg].second.second;
    ret = ssim[curVer][videoSegmentIdx[video][seg]];
    double maxValue = 0;
    for(auto ver : hotStoredVersion[video][seg]){
        if(reqVer <= ver){
            int nextBandwidth = bth + GetServiceTime(video, seg, ver);
...
void bandwidth_allocation(int sec){
    diskRequestSegment.assign(NUMBER_OF_DISK, vector<pair<int, pair<int, int> > >());
    ...
    while(pos != tail){
        ...
        int ver 				= pos->ver;
        diskRequestSegment[segment_allocation_disk[video][seg]].push_back({video, {seg, ver}});
        pos->seek = !pos->seek;
        pos = pos->next;
    }
    ...
    for(int d = 0; d < NUMBER_OF_DISK; d++){
        ...
        int ver = diskRequestSegment[d][0].second.second;
